...
object semiauto {
  ...
  inline final def deriveEncoder[A](using inline A: Mirror.Of[A]): Encoder[A] = Encoder.derived[A]
  inline final def deriveCodec[A](using inline A: Mirror.Of[A]): Codec[A] = Codec.derived[A]
}
